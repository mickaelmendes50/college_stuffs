...
__global__ void monteCarloPI_parallel(unsigned int *count, unsigned int N) {
    ...
    for (unsigned int i = tid; i < N; i += total_threads) {
    ...
    // Soma local_count em um único valor usando atomicAdd
...
int main() {
    ...
    monteCarloPI_parallel<<<blocks, threads>>>(d_count, N);
